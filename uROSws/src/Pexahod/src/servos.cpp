...
uint8_t update_servos(Leg *legs[]) {
    ...
    for (uint8_t leg = 0; leg < 6; leg++) {
        if (legs[leg]->is_enabled()) {
            ...
            printf("Coxa nr: %d - servo pos.: %d - ", legs[leg]->get_coxa_servo(), legs[leg]->get_coxa_position());
            printf("Femur nr: %d - servo pos.: %d - ", legs[leg]->get_femur_servo(), legs[leg]->get_femur_position());
            printf("Tibia nr: %d - servo pos.: %d\n\r", legs[leg]->get_tibia_servo(), legs[leg]->get_tibia_position());
